...
export default class HandWrite extends Component {
  ...
  componentWillMount () {
    let canvasName = this.state.canvasName
    let ctx = Taro.createCanvasContext(canvasName)
    this.setState({
      ctx: ctx
    })
  ...
  uploadScaleEnd = (e) => {
    ...
    var chirography = this.state.chirography
    chirography.unshift(currentChirography)
    this.setState({
      chirography
    })
    var linePrack = this.state.linePrack
  ...
  calcBethelLine = (line) => {
    ...
    this.setState({
      radius:
        Math.min(
          (time / len) * this.state.pressure + this.state.lineMin,
          this.state.lineMax
        ) * this.state.lineSize
    })
    line[0].r = this.state.radius
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (point.length == 3) {
        ...
        // console.log(a)
        // console.log(this.state.bethelPoint)
  ...
  // 重写
  retDraw = () => {
    this.state.ctx.clearRect(0, 0, 700, 730)
    this.state.ctx.draw()
  }
    ...
    return (
      <View className="wrapper">
        <View className="handBtn">
          <Image onClick={this.selectColorEvent} src={ selectColor === 'black' ? blackActive : black } class="{{ selectColor === 'black' ? 'color_select' : '' }} black-select" data-color="black"
